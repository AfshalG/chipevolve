...
3. Write synthesizable SystemVerilog. No delays, no `initial` blocks outside a
   ...
   Name the module after what it does, not after the directory. Put it at
   `rtl/<module>.sv` unless the project clearly uses another layout, and write it
   exactly once - do not leave a second copy at another path.
...
def system_prompt(mode: str, project: ProjectConfig, workspace_label: str) -> str:
    ...
    context = PROJECT_TEMPLATE.format(
        name=project.name,
        top=project.top or "(none yet - choose a name that fits the request)",
