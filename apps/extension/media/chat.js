...
const VALID_MODES = new Set(MODES.map((mode) => mode.id));

/** Never let an unknown mode reach the renderer; it used to print "undefined". */
function safeMode(value) {
  return VALID_MODES.has(value) ? value : "optimize";
}

const HEADINGS = { generate: "Generate RTL", review: "Review RTL", optimize: "Optimize RTL" };

const PLACEHOLDER = {
  generate: "Describe the module to write — ports, behaviour, timing…",
  review: "What should the review focus on?",
  optimize: "What should this generation try to improve?",
};

const SUGGESTIONS = {
  generate: [
    ["Write a parameterised synchronous FIFO", "Depth and width parameters, full/empty flags"],
    ["Add a 4-stage pipelined multiplier", "Valid/ready handshake, synchronous reset"],
  ],
  review: [
    ["Review this file for correctness and synthesis quality", "Lint and synthesis run as evidence"],
    ["Check every case statement for latch inference", "Findings land in the Problems panel"],
  ],
  optimize: [
    ["Reduce cell count without breaking the testbench", "One measured transformation per generation"],
    ["Shorten the critical path through the decode", "Recalls prior experiments before mutating"],
  ],
};
    ...
    .map((block, index) => {
      ...
      return block
        ...
        .map((paragraph) => {
          ...
          if (/^#{1,6}\s+/.test(paragraph)) return `<h4>${inline(paragraph.replace(/^#{1,6}\s+/, ""))}</h4>`;
...
function toolCard(row) {
  ...
  let indicator = "";
  if (row.status === "running") indicator = `<span class="spin"></span>`;
  ...
  return `<div class="tool ${awaiting ? "awaiting" : failed ? "error" : ""}">
      ${head}${row.open ? toolBody(row) : ""}${awaiting ? approvalRow(row) : ""}
    </div>`;
}

function approvalRow(row) {
  const target = (row.preview && row.preview.path) || (row.input && row.input.path) || "";
  const showDiff = TOOL_UI[row.name] && TOOL_UI[row.name].edit && target;
  return `<div class="approve">
      ${showDiff ? `<button class="open-diff" data-diff="${attr(target)}">Open diff</button>` : ""}
...
function toolBody(row) {
  ...
  if (meta.diff || preview.diff) return `<div class="body">${renderDiff(meta.diff || preview.diff)}</div>`;
  if (preview.error) return `<div class="body"><pre>${escapeHtml(preview.error)}</pre></div>`;
  if (row.name === "score_candidate" && meta.fitness) return scoreBody(meta);
  if (row.name === "report_findings" && meta.findings) return meta.findings.map(findingRow).join("");
  if (row.name === "run_synthesis" && meta.metrics) return metricsBody(meta);
  if (row.detail) return `<div class="body"><pre>${escapeHtml(row.detail)}</pre></div>`;
  return "";
}

function metricsBody(meta) {
  const cells = (meta.metrics || {}).cell_count;
  const before = (meta.baseline || {}).cell_count;
  const change = before && cells ? ((cells - before) / before) * 100 : null;
  const cls = change == null ? "" : change < 0 ? "good" : change > 0 ? "bad" : "";
  return `<div class="metrics">
      ...
      }</strong><span>Delta</span></div>
    </div>`;
}

function scoreBody(meta) {
  const improvement = (meta.fitness || {}).improvement_percent;
  ...
  return `<div class="metrics">
      <div class="${meta.accepted ? "good" : "bad"}"><strong>${
        meta.accepted ? "ACCEPT" : "REJECT"
      }</strong><span>Decision</span></div>
      ...
      ${gate("Lint", v.lint_passed)}
      ${gate("Sim", v.simulation_passed)}
      ${gate("Synth", v.synthesis_passed)}
      ${gate("Integrity", v.protected_files_intact)}
    </div>`;
}
...
function renderRow(row) {
  switch (row.kind) {
    ...
    case "text": {
      const body = markdown(row.text);
      return body.trim() ? `<div class="assistant">${body}</div>` : "";
    }
    case "thinking":
      if (!row.text.trim()) return "";
      return `<div class="thinking"><div class="head" data-toggle="${row.id}">${
        row.open ? "▾" : "▸"
      } Thinking</div>${row.open ? `<div class="body">${escapeHtml(row.text)}</div>` : ""}</div>`;
    case "tool":
      return row.name ? toolCard(row) : "";
    case "error":
...
function renderTimeline() {
  if (!state.rows.length && !state.pendingApply) {
    ...
    return `<div class="empty">
        <h3>${escapeHtml(HEADINGS[state.mode] || HEADINGS.optimize)}</h3>
        <div>Every claim is checked by Verilator and Yosys before it reaches you.</div>
        ${state.focus ? `<div class="focus">${escapeHtml(state.focus)}</div>` : ""}
  ...
  return `${rows}<div class="tool awaiting">
      ...
      <div class="body"><div class="finding"><div class="detail">
        The edits live in a generation workspace, so your working tree stayed clean.
        Applying copies ${files} into the project.
      </div></div></div>
      <div class="approve">
...
function renderTaskCard() {
  ...
  return `<div class="taskcard">
      <div class="label">${escapeHtml(state.task.mode)}${
        state.task.workspace ? ` · ${escapeHtml(state.task.workspace)}` : ""
      }</div>
      <div class="text">${escapeHtml(state.task.message)}</div>
      <div class="meters">
        ${state.project ? `<span>${escapeHtml(state.project)}</span>` : ""}
...
function renderComposer() {
  const modes = MODES.map(
    (mode) => `<button data-mode="${mode.id}" class="${state.mode === mode.id ? "active" : ""}">${mode.label}</button>`,
  ).join("");
  ...
  return `<div class="composer">
      ...
      <div class="inputwrap">
        <textarea id="prompt" rows="2" placeholder="${attr(
          PLACEHOLDER[state.mode] || PLACEHOLDER.optimize,
    ...
    ${
      state.online
        ...
        : `<div class="offlinebar">
             ...
             <span class="label">${escapeHtml(state.offlineReason || "Backend offline")}</span>
             <button class="icon-button" id="start-backend">Start</button>
           </div>`
    }
    ${renderTaskCard()}
...
function onEvent(event) {
  // The backend stamps a monotonic seq. A bounded seen-set drops replays from a
  // stray second connection without mistaking a restarted backend for duplicates.
  ...
  switch (event.type) {
    ...
    case "chat.thinking": {
      ...
      if (row) row.text += payload.text;
      else state.rows.push({ kind: "thinking", id: `think-${nextRowId()}`, text: payload.text, open: false });
      break;
    }
    case "chat.block_end":
    ...
    case "chat.tool_start":
      if (findRow(payload.tool_use_id)) break;
      state.rows.push({
        kind: "tool",
    ...
    case "chat.done":
      state.busy = false;
      if (payload.reason === "cancelled") state.rows.push({ kind: "done", text: "Task cancelled." });
      else if (payload.detached && (payload.edited || []).length) state.pendingApply = payload.edited;
      break;
    default:
      return;
  }
...
document.addEventListener("click", (nativeEvent) => {
  ...
  if (target.dataset.mode) {
    state.mode = safeMode(target.dataset.mode);
    save();
    return schedule();
  }
  ...
  if (target.id === "send") return send();
  if (target.id === "stop") return vscode.postMessage({ command: "cancel" });
  if (target.id === "start-backend") return vscode.postMessage({ command: "startBackend" });
});
...
window.addEventListener("message", (nativeEvent) => {
  ...
  switch (message.type) {
    case "status":
      state.online = Boolean(message.online);
      state.offlineReason = message.reason || null;
      state.project = message.project || null;
      state.focus = message.focus || null;
      if (!state.online) state.busy = false;
      break;
    case "event":
    ...
    case "setMode":
      state.mode = safeMode(message.mode);
      save();
      break;
    case "newTask":
      newTask();
